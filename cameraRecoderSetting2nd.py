...
isRecordedAsVideo = False   # ビデオとして記録するかどうかのフラグ：Trueの場合にはビデオ(avi)として記録、Falseの場合には画像(jpg)として記録する。
diff_rate_threshold = 0.05  # 差分率の閾値：0の場合には常時録画、1の場合には超音波距離センサに近接した時に録画開始、0から1の間は差分率が閾値を超えた場合に録画開始。
diff_level_threshold = 30   # 差分レベルの閾値：差分レベルが閾値を超えた場合に差分レベルが閾値を下回ったら録画を開始する。
rec_fps = 1                 # 記録するフレームレート
rec_time = 10               # 記録時間（秒）
cameras = ['http://192.168.11.3:81/stream', 'http://192.168.11.4:81/stream'] #['http://192.168.115.57:81/stream'] #, 'http://192.168.115.110:81/stream']              # カメラの指定：カメラが2台の時は[0,1]、一方がネットワークカメラ（ESP32_S3_WROOM）の場合は [ 0, 'http://192.168.11.10:81/stream'] という風に指定する。
...
line_access_token = ''      # LINE Notifyのアクセストークン 例：'x2mY2Joizz27DW8X9AHP7zwCFuKvcqjUkM8tLi9t4St'
