...
diff_rate_threshold = 1     # 差分率の閾値：0の場合には常時録画、1の場合には超音波距離センサに近接した時に録画開始、0から1の間は差分率が閾値を超えた場合に録画開始。
diff_level_threshold = 30   # 差分レベルの閾値：差分レベルが閾値を超えた場合に差分レベルが閾値を下回ったら録画を開始する。
rec_fps = 1                 # 記録するフレームレート
rec_time = 5                # 記録時間（秒）
cameras = [0]               # カメラの指定：カメラが2台の時は[0,1]、一方がネットワークカメラ（ESP32_S3_WROOM）の場合は [ 0, 'http://192.168.11.10:81/stream'] という風に指定する。
record_resolution = 'VGA'   # 記録解像度
display_resolution = 'VGA'  # 表示解像度
serial_port = 'COM8'        # シリアルポートの設定：マイコン経由で超音波センサを接続したポート（例：’COM11’）を指定する。未接続の場合は’’’またはNoneを指定する。
